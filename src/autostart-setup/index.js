...
(async () => {
  ...
  try {
    ...
    await retry(3, async () => {
      ...
      const lastinput = await lunaCall(
      ...
      if (lastinput.stdoutString) {
        ...
        if (lastinputPayload.appId) {
          ...
          if (lastinputPayload.appId !== "com.iptv.webos") {
  ...
  try {
    ...
    await retry(3, async () => {
      ...
      const res = await lunaCall("luna://com.webos.service.eim/addDevice", {
        ...
        label: "IPTV WebOS", // required for webOS 3.4 at least
        description: "IPTV WebOS", // required for webOS 3.4 at least
      });
      logres(res);
    });
  } catch (err) {
  ...
  try {
    ...
    await retry(3, async () => {
      ...
      const res = await lunaCall(
        ...
        {
          command:
            "if [[ ! -d /var/lib/webosbrew/eim ]]; then cp -r /var/lib/eim /var/lib/webosbrew/eim && echo cp ok || echo cp failed; fi ; if ! findmnt /var/lib/eim; then mount --bind /var/lib/webosbrew/eim /var/lib/eim && echo mount overlay ok || echo mount overlay failed; fi",
        },
      );
